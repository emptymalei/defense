...
\begin{itemize}
    \item[] Hamiltonian with matter interaction in flavor basis  ($\omega_{\mathrm{v}}=\delta m^2 \big/ 2E$):
        \tikz[na] \node[coordinate] (n1) {};
\end{itemize}
...
\only<2->{
\begin{equation*}
   i(\partial_t + {\boldsymbol v} \cdot \boldsymbol{\nabla}_r  ) \epsilon (\boldsymbol v) = \colorbox{blue!50}{$ v^\mu$} ( \colorbox{red!50}{$\Lambda$} + \colorbox{ao!60}{$\Phi$} )_\mu - \int d\Gamma' v^\mu v'_\mu \colorbox{armygreen!70}{$G(\boldsymbol v')$} \epsilon(\boldsymbol v')
\end{equation*}
}
...
\begin{frame}{Dispersion Relation}
    ...
    \begin{tcolorbox}
       ...
       \omega =\colorbox{red!50}{$\frac{1}{4}(I_0-I_2)$}, \quad \colorbox{blue!50}{$-\frac{1}{4}\left(I_0-I_2\pm \sqrt{ (I_0-2I_1+I_2)(I_0+2I_1+I_2) }\right)$}
        ...
        \end{tcolorbox}
\begin{itemize}
    \item \colorbox{red!50}{MAA solution}
    \item \colorbox{blue!50}{MZA solution}
\end{itemize}
...
\pause
   \begin{tcolorbox}
      \centering
      Gap and instability correspondance does NOT hold in three-zenith-angle model case.
   \end{tcolorbox}
...
\begin{frame}{Dispersion Relations}
 ...
 Box spectrum: $-0.1$ within range $u\in [-1,-0.3)$ and $1$ within range $u\in [-0.3,1]$
   ...
   \begin{figure}
    ...
    \pause
    \begin{tcolorbox}
      Instabilities occur even without dispersion relation gaps.
    \end{tcolorbox}
   \end{figure}
...
\begin{itemize}
   \item Neutrino oscillation instability corresponds to gaps of dispersion relations for two-zenith-angle models.
   \item It can NOT be generalized to multiple-zenith-angle models nor continuous emission models.
...
I am very thankful to my advisor Professor Huaiyu Duan, as well as my colleagues Dr. Sajad Abbar, and Dr. Shashank Shalgar, and Joshua Martin, for all the help in both research and life.

My research is supported by DOE.
